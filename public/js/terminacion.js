...
$(document).ready(function() {
    ...
    $("#codprove").change(function() {
        ...
        $.post(
                "../reporte/terminacion/nroSolicitudProveedor",
                {codprove: $(this).val()},
        function(res) {
            if (res != 0) {
    ...
    var table = $("#dtservicios").DataTable({
        ...
        //"paging": true,
        "ajax": {
            "url": "../reporte/terminacion/generar",
            "data": function(d) {
                return $.extend({}, d, {
                    codprove: $("#codprove").val(),
        ...
        "columns": [
            ...
            {data: "VALOR_UNITARIO", render: $.fn.dataTable.render.number(',', '.', 2, '$')},
            {"data": "CANTIDAD", "name": "CANTIDAD"},
            {"data": "VALOR_SIN_IVA", render: $.fn.dataTable.render.number(',', '.', 2, '$')},
            {"data": "IVA", render: $.fn.dataTable.render.number(',', '.', 2, '$')},
            {"data": "VALOR_IVA", render: $.fn.dataTable.render.number(',', '.', 2, '$')},
    ...
    $("#gen_pdf").click(function(e) {
        ...
        if (validarCampos()) {
            myApp.showPleaseWait();
            $.post(
                    "../reporte/terminacion/generarPdf",
                    {
                        codprove: $("#codprove").val(),
                        fechaini: $("#fechaini").val(),
                        fechafin: $("#fechafin").val(),
