...
$(document).ready(function() {
     ...
     */
    var table = $("#dtparametros").DataTable({
        "processing": true,
        "serverSide": true,
        "responsive": true,
        "autoWidth": false,
        //"paging": true,
        "ajax": {
            "url": "../parametro/parametro/obtenerParametros",
            "type": "POST"
        },
        //"data": obj.servicios,
        "columns": [
            {"data": "COD_PARAM", "name": "COD_PARAM"},
            {"data": "DESCR", "name": "DESCR"},
            {"data": "PARAM_DESCR", "name": "PARAM_DESCR"},
            {"data": "VLR_N", "name": "VLR_N"},
            {"data": "VLR_C", "name": "VLR_C"},
            {"data": "ACT", "name": "ACT"},
            {"data": "FECHA_CREA", "name": "FECHA_CREA"},
            //{"data": "FECHA_SERVICIO", "name": "FECHA_SERVICIO"},
            //{"data": "NRO_FACTURA", "name": "NRO_FACTURA"},
            {"data": "COD_PARAM", "name": "COD_PARAM", "width": "50px"}
     ...
     */
    $('#dtparametros tbody').on('click', 'a', function() {
        var enlace = $(this);
        var data = table.row(enlace.parents('tr')).data();
        //alert( 'You clicked on ' + data.DESCR + ' row' );

        $("#codigo").val(data.COD_PARAM);
        $("#cod_old").val(data.COD_PARAM);
        $("#descri").val(data.DESCR);
        $("#tiparam").val(data.TIPO_PARAM);
        $("#vlrn").val(data.VLR_N);
        $("#vlrc").val(data.VLR_C);
        $("#activo").val(data.ACT);
    ...
    $("#guardar").click(function(e) {
        ...
        if (validarCampos()) {
            ...
            if ($("#guardar").val() === 'Guardar') {
                $.post(
                        "../parametro/parametro/guardarParametro",
                        {
                            codigo: $("#codigo").val(),
                            descri: $("#descri").val(),
                            tiparam: $("#tiparam").val(),
                ...
                $.post(
                        "../parametro/parametro/actualizarParametro",
                        {
                            codigo: $("#codigo").val(),
                            cod_old: $("#cod_old").val(),
                            descri: $("#descri").val(),
...
function quitarError() {
    ...
    $("#divtiparam").attr('class', 'form-group');
    //$("#divvlrn").attr('class', 'form-group');
    //$("#divvlrc").attr('class', 'form-group');
...
function validarCampos() {
    ...
    if ($("#codigo").val() === '') {
        ...
        flag = false;
    } else if ($("#tiparam").val() === '-1' && flag) {
        $("#divtiparam").attr('class', 'form-group has-error');
        flag = false;
