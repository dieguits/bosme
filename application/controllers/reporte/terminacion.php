...
class Terminacion extends CI_Controller {
    ...
    public function index() {
        $this->load->helper('form');
        $datos = array(
            'content' => 'reporte/terminacion',
            'script' => '<script type="text/javascript" src="' . base_url() . 'public/js/terminacion.js"></script>',
            'titulo' => '<h2>BOSME</h2><p>BOLSA DE SOLICITUDES DE MERCADEO</p>',
            'controlador' => 'terminacion',
    ...
    public function generar() {
        ...
        $data = $this->servicio_model->reporteTerminacion($filtro);
        $datos = $this->servicio_model->reporteTerminacionFiltrado($filtro);
    ...
    public function generarPdf() {
        ...
        $data = array(
            'datos' => $this->servicio_model->reporteTerminacionPdf($filtro),
            'fechaini' => $this->input->post('fechaini'),
            'fechafin' => $this->input->post('fechafin'),
        ...
        $filename = 'termina'.date('d_m_y_h:i').'.pdf';
        ...
        $vista = $this->load->view("pdf", $data, true);
